...
for node in $nodes; do
    LOGIN_MSG=$(ssh core@$node "docker login docker.cloud.wso2.com -u $USERNAME -p $PASSWORD")
    ...
    ssh core@$node "docker pull docker.cloud.wso2.com/sshd-kubernetes:1.0.0 &&
		docker pull docker.cloud.wso2.com/rsync-kubernetes:1.0.0 &&
